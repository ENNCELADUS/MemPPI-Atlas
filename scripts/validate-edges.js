...
function loadNodeSet(nodesCsvPath) {
  ...
  if (proteinIdx === -1)
    throw new Error("protein column not found in nodes.csv");
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!nodeSet.has(p1) || !nodeSet.has(p2)) {
      ...
      if (missingProteinSamples.length < 5) {
        missingProteinSamples.push({
          edge,
          p1,
          p2,
          missing: { p1: !nodeSet.has(p1), p2: !nodeSet.has(p2) },
        });
      }
    }
  }
...
(function main() {
  ...
  if (!fs.existsSync(NODES_CSV) || !fs.existsSync(EDGES_CSV)) {
    console.error(
      "Expected CSV files not found in data/supabase-import/. Run prepare-csvs-for-import.js first."
    );
    process.exit(1);
  }
  ...
  console.log(
    `Edges referencing missing proteins: ${report.missingProteinCount}`
  );
