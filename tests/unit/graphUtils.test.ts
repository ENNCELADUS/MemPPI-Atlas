...
describe('graphUtils', () => {
  ...
  describe('getEdgeColor', () => {
    ...
    it('uses experimental color when positiveType=experimental', () => {
      expect(getEdgeColor({ ...baseEdge, positiveType: 'experimental' })).toBe(edgeColors.experimental);
    });

    it('uses enriched color when enriched tissue present', () => {
      expect(getEdgeColor({ ...baseEdge, enrichedTissue: 'Brain' })).toBe(edgeColors.enriched);
    });
