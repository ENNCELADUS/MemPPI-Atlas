...
export default async function handler(
  ...
  try {
    ...
    if (!proteinsParam || proteinsParam.trim() === '') {
      return res.status(400).json({ error: 'Missing required parameter: proteins' });
    }
    ...
    if (queryProteins.length === 0) {
      return res.status(400).json({ error: 'Invalid proteins parameter: must contain at least one protein ID' });
    }
    ...
    if (preferExperimental) {
      ...
      const { data: expEdges, error: expErr } = await supabase
        ...
        .or(
          queryProteins.map(p => `protein1.eq.${p},protein2.eq.${p}`).join(',')
        )
        .limit(maxEdges);
    ...
    if (remaining > 0) {
      ...
      if (predErr) {
        ...
        const newEdges = typedPredEdges.filter(e => !existingEdgeIds.has(e.edge));
    ...
    if (edges.length === 0) {
      ...
      if (nodeCheckErr) {
        ...
        return res.status(500).json({ error: 'Failed to check proteins in database' });
      }

      if (!existingNodes || existingNodes.length === 0) {
        return res.status(404).json({ error: 'None of the queried proteins exist in the dataset' });
      }
    ...
    if (allProteinIds.length > maxNodes) {
      ...
      const neighborIds = allProteinIds.filter(id => !queryProteins.includes(id));
      limitedProteinIds = [...queryProteins, ...neighborIds.slice(0, maxNodes - queryProteins.length)];
      nodesTruncated = true;

      // Filter edges to only include those connecting limited nodes
      const limitedProteinIdsSet = new Set(limitedProteinIds);
      edges = edges.filter(
        edge => limitedProteinIdsSet.has(edge.protein1) && limitedProteinIdsSet.has(edge.protein2)
      );
    }
